...
try:
    ...
    psutil = None
    print("Warning: psutil not available. ComfyUI status monitoring will be disabled.")
...
<style>
    ...
    /* Blue container for Civitai API input */
    .comfy-root .civitai-container {
        background: linear-gradient(180deg, #0A5CFF 0%, #0747D1 100%);
        border: 2px solid #0639B2;
        border-radius: 12px;
        padding: 12px 20px;
    ...
    /* Enhanced Get API Key link styling */
    .comfy-root .civitai-get-key-link a {
        color: white !important;
        font-size: 18px !important;
        font-weight: bold !important;
    ...
    .comfy-root .civitai-get-key-link a:hover {
        text-decoration: underline !important;
        color: #F0F0F0 !important;
    }

    /* White input field inside blue container */
    .comfy-root .civitai-container input,
    .comfy-root .civitai-container .widget-text {
        background: white !important;
        border: none !important;
        color: #1A237E !important;
        border-radius: 8px !important;
...
# Civitai token input widget (bound to Python) - load existing API key
civitai_token_widget = widgets.Text(
    value=load_api_key(),  # Initialize with existing key
...
try:
    ...
    if 'civit-token-input' not in civitai_token_widget._dom_classes:
        civitai_token_widget._dom_classes = civitai_token_widget._dom_classes + ['civit-token-input']

# Add observer to automatically save API key changes
...
# Create "Get API Key" link with enhanced styling
...
# Create the blue container with link and input field inside
civitai_container_content = widgets.HBox([get_api_key_link, civitai_token_widget], 
                                       layout=widgets.Layout(align_items='center', gap='20px', width='100%'))

# Wrap in blue container
civitai_input_row = widgets.VBox([civitai_container_content], 
                                layout=widgets.Layout(align_items='center', justify_content='center', 
                                                     width='100%', margin='6px 0px'))
try:
    civitai_input_row.add_class('civitai-container')
except Exception:
    if 'civitai-container' not in getattr(civitai_input_row, '_dom_classes', []):
        civitai_input_row._dom_classes = getattr(civitai_input_row, '_dom_classes', []) + ['civitai-container']
...
if psutil is not None:
    ...
    print("ComfyUI status monitoring disabled (psutil not available)")
